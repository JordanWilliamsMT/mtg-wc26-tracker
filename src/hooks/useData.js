...
export function useData() {
  ...
  const refresh = useCallback(async (keys = ['fixtures','standings','odds','participants','moments','meta']) => {
    try {
      const results = await Promise.allSettled(keys.map(k => fetchKey(k)))
      const map = Object.fromEntries(keys.map((k, i) => [k, results[i].status === 'fulfilled' ? results[i].value : null]))
  ...
  function getLeaderboard() {
    ...
    return pList.map(p => {
      ...
      const totalWinChance = Math.round((teamOdds[0] || 0) + (teamOdds[1] || 0))
      ...
      const rounds = ['Group Stage', 'Round of 32', 'Round of 16', 'Quarter-finals', 'Semi-finals', 'Final', 'World Cup Final']
      const furthestRound = (p.teams || []).reduce((best, t) => {
        const teamMatches = fixtureList.filter(f =>
          (f.status === 'FT' || f.status === 'AET' || f.status === 'PEN') &&
          (f.homeTeam === t || f.awayTeam === t)
        )
        if (!teamMatches.length) return best
        const lastMatch = teamMatches[teamMatches.length - 1]
        const roundIdx = rounds.findIndex(r => lastMatch.round?.includes(r.split(' ')[0]))
    ...
    }).sort((a, b) => b.winChance - a.winChance || a.name.localeCompare(b.name))
  }

  // ── Tonight's stakes ───────────────────────────────────────────────────────
  function getTonightStakes() {
    const todayStr = new Date().toISOString().slice(0, 10)
    const todayFixtures = (fixtures?.fixtures || []).filter(f => f.date?.startsWith(todayStr))
    const pList = participants?.participants || []
