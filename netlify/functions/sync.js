...
function generateMoment({ type, team, participant, opponent, score }) {
  const templates = {
    ...
    goodGame: [`⚽ ${team} ${score} ${opponent} — what a game!`],
  };
...
export default async function handler(req) {
  ...
  try {
    ...
    const existingMomentsData = await store
    ...
    const participantsData = await store
    ...
    const liveOrSoon = gamesToday.filter((f) =>
      ["NS", "1H", "HT", "2H", "ET", "BT", "P", "INT"].includes(f.status),
    );
